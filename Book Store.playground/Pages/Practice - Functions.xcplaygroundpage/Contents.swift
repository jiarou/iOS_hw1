...
print(bookList[0]["author"] ?? <#default value#>)


func distinctAuthors() -> Set<String> {
    var authorArray = Array<String>()

    for number in 0..<(bookList.count){
        
          authorArray.append(bookList[number]["author"] ?? <#default value#>)
        
    }
    let authorSet = Set(authorArray)
    return authorSet

      }

print(distinctAuthors())
...
Double(bookList [0]["price"]! )!
func totalBookPrice() -> Double {
    var total:Double = 0
    for money in  0..<(bookList.count){
     let getPrice =  Double(bookList [money]["price"]! )!
      total += getPrice
...
func getBook(at index: Int) -> (title: String, author: String, price: Double)? {
    //typealias Book = (title: String, author: String, price: Double)
    guard index <  bookList.count else {
      return nil
    }
    let bookInformation =  bookList[index]
    guard let gettitle = bookInformation["title"], let title = String(gettitle) else {
       
        
        return nil }
    guard let getauthor = bookInformation["author"],let author = String(getauthor) else {
               return nil }
    guard let getprice = bookInformation["price"],let price = Double(getprice) else {
